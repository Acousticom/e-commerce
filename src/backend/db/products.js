
export const products = [
  {
    id: 1,
    productName: "Men's Solid Button Down Collar Shirt",
    description:
      "Light Blue floral printed Casual shirt ,has a cutaway collar, button placket, long regular sleeves, curved hem",
    productId: 4514109,
    quantity: 1,
    price: 1373,
    mrp: 2495,
    rating: 4.5,
    discountDisplayLabel: "45% off",
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "30 minutes",
    brand: "Tommy Hilfiger",
    category: "Shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/5/4/tr:w-300,/540bf1fTBH-BEZOS-MR_1.jpg"
  },
  {
    id: 2,
    productName: "Premium Black and White Slim Fit Tie Dye Rayon Shirt",
    description:
      "An absolute snag, this piece has our heart. It is for people who love striking a mix between stylish and comfortable. this tie-dye art is a steal deal especially if you are obsessed with white like we are. made of 100% viscose fabric this piece is extremely breezy and super soft to carry.",
    productId: 6212141,
    quantity: 1,
    price: 1275,
    mrp: 1499,
    discountDisplayLabel: "15% off",
    rating: 4.0,
    section: "Men",
    inStock: false,
    fastDelivery: false,
    trending: false,
    deliveryTime: "10 days",
    brand: "JAVINISHKA",
    category: "Shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/e/f/tr:w-480,/ef2feebJAVINTDS01_1.jpg?rnd=20200526195200"
  },
  {
    id: 3,
    productName: "Off Duty Mom Flare Wide Leg High Rise Jeans",
    description:
      "Update your denim with these mom flare wide leg denims. featuring a light wash denim and a wide leg flare, we are obsessed. team them with a simple white top for a combo that can be dressed up or down. buy them now, wear them forever",
    productId: 3956861,
    quantity: 1,
    price: 1699,
    mrp: 2199,
    discountDisplayLabel: "23% off",
    rating: 3.2,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "30 minutes",
    brand: "Levi's",
    category: "Jeans",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/o/d/tr:w-480,/od-dn-wl-1018_1_4be3a210.jpg?rnd=20200526195200"
  },
  {
    id: 4,
    productName: "Batman: Gotham's Knight Oversized T-shirts For Mens",
    description:
      "Look like a handsome hunk in this t-shirt from The Souled Store. This dapper piece flaunts a yellow colour and feels soft on the skin. Style it shorts and sneakers and amp up your casual attire like never before.",
    productId: 6789512,
    quantity: 1,
    price: 879,
    mrp: 1499,
    discountDisplayLabel: "41% off",
    rating: 3.8,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "40 min",
    brand: "The Souled Store",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/1/5/tr:w-480,/15743bcNYK_THECB00013495_1.jpg?rnd=20200526195200",
    
  },

  {
    id: 5,
    productName: "Men Solid Navy Shirt",
    description:
      "Update your look with this Navy Solid Slim Fit shirt from Allen Solly by Allen Solly and enjoy the attention.",
    productId: 8873068,
    quantity: 1,
    price: 1060,
    mrp: 1799,
    discountDisplayLabel: "41% off",
    rating: 3.8,
    section: "Men",
    inStock: false,
    fastDelivery: false,
    trending: false,
    deliveryTime: "4 days",
    brand: "Allen Solly",
    category: "Shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/e/6/tr:w-480,/e6f2871ASSFQMOBK06245_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 6,
    productName: "Looney Tunes: Daffy In Space Jam Oversized T-shirts For Mens",
    description:
      "Add some coolness to your style with this t-shirt from The Souled Store. This dapper piece flaunts a pink colour and feels soft on the skin. Style it with jeans and sneakers for a perfect brunch look.",
    productId: 6789493,
    quantity: 1,
    price: 1049,
    mrp: 1400,
    discountDisplayLabel: "30% off",
    rating: 3.2,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "2 hour",
    brand: "The Souled Store",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/8/2/tr:w-480,/829baf8KL_THECB00013476_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 7,
    productName: "Men Multi-color Slim Fit Stripe Full Sleeves Casual Shirts",
    description:
      "Update your look with this multi-color casual shirt from Allen Solly. Made from a comfortable material with a slim fit, it has a collar neck and full sleeves. Team with a pair of chinos and loafers for a sleek look.",
    productId: 8573339,
    quantity: 1,
    price: 2225,
    mrp: 2499,
    discountDisplayLabel: "11% off",
    rating: 3.9,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "2 days",
    brand: "Allen Solly",
    category: "Shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/5/d/tr:w-480,/5de7bceASSFQSPP140419_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 8,
    productName: "Official Minions One In A Minion Oversized T-shirt For Mens",
    description:
      "Official Licensed Minions Oversized T-Shirt.Souledsters! Make way, the Minion Army is here!Everyone's favourite, banana-loving characters from Despicable Me are here to add a dash of fun and a whole lot of quirk to your wardrobe.Of course our new Minions Collection was approved by Stuart, Bob and Kevin, who asked us for a whole lot of Paneer Tikka Masala before signing off on the designs!Designed for comfort, this new range from The Souled Store features premium quality t-shirts and more that make for great conversation starters.Buy exclusive Minions merch only from The Souled Store",
    productId: 5650698,
    quantity: 1,
    price: 839,
    mrp: 1499,
    discountDisplayLabel: "11% off",
    rating: 3.2,
    section: "Men",
    inStock: true,
    fastDelivery: false,
    trending: true,
    deliveryTime: "3 days",
    brand: "The Souled Store",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/2/a/tr:w-480,/2af9af8KL_THECB00007078_1.jpg?rnd=20200526195200",
    
  },

  {
    id: 9,
    productName: "Womens Baggy Fit Jeans",
    description:
      "Look stylish when you step out wearing these attractive Levi's®Women's Baggy Fit Jeans. Featuring a distressed pattern, this blue pair of jeans makes you look distinctive. Made from durable cotton material, these jeans boast a flattering baggy fit.",
    productId: 9919213,
    quantity: 1,
    price: 2129,
    mrp: 3899,
    discountDisplayLabel: "45% off",
    rating: 4.8,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "5 hour",
    brand: "Levi's",
    category: "Jeans",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/2/e/tr:w-480,/2ef9d3dA3941-0001_1.jpg?rnd=20200526195200",
   
  },
  {
    id: 10,
    productName: "Men Black Solid Polo Neck T-Shirt",
    description:
      "Ace weekend dressing in this Black Solid Polo Neck T-shirt from Allen Solly by Allen Solly.",
    productId: 10380729,
    quantity: 1,
    price: 1157,
    mrp: 1299,
    discountDisplayLabel: "11% off",
    rating: 2.5,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "1 hours",
    brand: "Allen Solly",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/b/1/tr:w-480,/b1c0d95ASKPNRGFT55946_1.jpg?rnd=20200526195200",
   
  },
  {
    id: 11,
    productName: "Originals Oversized Red Oversized T-shirt For Mens ",
    description:
      "It's no secret that we've been making the best damn casual wear in India, since our inception in 2013!We've grown fourfold over the years, but we have still remained true to our roots. True to making the best quality superhero, TV show and movie merchandise for fans - by fans! Our new range of The Souled Store Originals Merchandise is an ode to the years that flew by. It is a special dedication to the people who've made us. And we'd love to see you flex in this supremely comfortable The Souled Store t-shirt. ",
    productId: 5650762,
    quantity: 1,
    price: 839,
    mrp: 1499,
    discountDisplayLabel: "44% off",
    rating: 3.0,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: " minutes",
    brand: "The Souled Store",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/a/d/tr:w-480,/ad2f3bcNYK_THECB00007142_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 12,
    productName: "Grey Tie Dye Cotton Shirt",
    description:
      "Ace the fashion game this season with evergreen tye-dye prints bought to you by Hatheli via its classy shirts. Made from cotton fabric, the shirt gives you guaranteed comfort and high standards of quality",
    productId: 4681212,
    quantity: 1,
    price: 1980,
    mrp: 2199,
    discountDisplayLabel: "10% off",
    rating: 4.8,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "3 hour",
    brand: "Tommy Hilfiger",
    category: "Shirts",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/e/7/tr:w-480,/e7ac583358M-SHGY_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 13,
    productName: "Black Shirt",
    description:
      "Add a feminine touch to your business casual look in this Black shirt from Allen Solly Woman by Allen Solly. Tailored for absolute comfort, this design is made from cotton with a hint of stretch. Team this Long Sleeves shirt with grey trousers and pumps to seal your look.",
    productId: 7283230,
    quantity: 1,
    price: 1602,
    mrp: 1799,
    discountDisplayLabel: "11% off",
    rating: 4.8,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "1 hour",
    brand: "Allen Solly",
    category: "Shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/a/3/tr:w-480,/a3e2df8AHSFGRGPD99920_1.jpg?rnd=20200526195200",
    
  },

  {
    id: 14,
    productName: "Maroon One Shoulder Bodycon Maxi Dress",
    description:
      "Create a bold look as you wear this maroon one shoulder bodycon maxi dress is going to be your next brunch pick from Label RSVP. Crafted using polyester material, it features a side closure and one shoulder Design. Style it up with strappy nude heels a small sling bag and a messy hair do is all you need for a complete look!",
    productId: 7741719,
    quantity: 1,
    price: 3651,
    mrp: 4295,
    discountDisplayLabel: "15% off",
    rating: 4.9,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "30 minutes",
    brand: "Zara",
    category: "Dresses",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/f/1/tr:w-480,/f1b6cf8D20162_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 15,
    productName: "Orange Handwoven Pure Cotton Tant Saree Without Blouse",
    description:
      "Pamper the Women in you with this beautiful orange saree from Charukriti. This saree is made of cotton fabric which is highlighted with a beautiful weaving pattern. The saree has a hand-woven pattern, which adds gorgeous sophistication to your look. You can wear this saree for casual outings, office, regular social meets and other homely events. Pair it with traditionally stylish accessories to complete the look.",
    productId: 9753185,
    quantity: 1,
    price: 1375,
    mrp: 5500,
    discountDisplayLabel: "75% off",
    rating: 2.3,
    section: "Women",
    inStock: true,
    fastDelivery: false,
    trending: false,
    deliveryTime: "4 days",
    brand: "Sare mall",
    category: "Saree",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/a/r/tr:w-480,/ar2tt046_1_a679872c.jpg?rnd=20200526195200",
   
  },

  {
    id: 16,
    productName: "Blue Printed A-Line Top",
    description:
      "Blue floral printed A-line top, has a mandarin collar, embellished neckline, 3/4th elasticated sleeves and gathers detail.",
    productId: 7627499,
    quantity: 1,
    price: 1125,
    mrp: 1499,
    discountDisplayLabel: "25% off",
    rating: 2.2,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: false,
    deliveryTime: "45 minutes",
    brand: "QOMN",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/a/0/tr:w-480,/a0ee88dQOMN_T1200-52_1.jpg?rnd=20200526195200",
    
  },

  {
    id: 17,
    productName: "Blue Jeans",
    description:
      "Accentuate your style in these blue jeans from Allen Solly and look stylish. They are made of high-quality material, this pair is a must-have. Team up with a t-shrit and shoes to complete the look.",
    productId: 7910440,
    quantity: 1,
    price: 3160,
    mrp: 3999,
    discountDisplayLabel: "21% off",
    rating: 4.9,
    section: "Men",
    inStock: true,
    fastDelivery: false,
    trending: true,
    deliveryTime: "3 days",
    brand: "Allen Solly",
    category: "Jeans",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/6/7/tr:w-480,/6764b73ABFRL-ALDNVSKFG14359_1.jpg?rnd=20200526195200",
   
  },

  {
    id: 18,
    productName: "Beige Three Piece (Set of 3)",
    description: "Georgette tube top and flared pants with shrug",
    productId: 6340929,
    quantity: 1,
    price: 3999,
    mrp: 3999,
    discountDisplayLabel: "0% off",
    rating: 4.2,
    section: "Women",
    inStock: true,
    fastDelivery: false,
    trending: false,
    deliveryTime: "2 days",
    brand: "Lavanya",
    category: "Suit",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/1/8/tr:w-480,/1856b78LBL101KS484_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 19,
    productName:
      "Solid Women Black Pleated Kurta Trousers with Dupatta (Set of 3)",
    description:
      "Women Black Pleated Kurta with Trousers & With Dupatta Solid A-line shape Anarkali style Halter neck, sleeveless no sleeves Floor length with flared hem Viscose rayon knitted and woven fabric Solid Trousers Elasticated waistband comes with dupatta",
    productId: 9732537,
    quantity: 1,
    price: 1485,
    mrp: 4499,
    discountDisplayLabel: "65% off",
    rating: 3.2,
    section: "Women",
    inStock: true,
    fastDelivery: false,
    trending: true,
    deliveryTime: "3 days",
    brand: "Lavanya",
    category: "Suit",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/b/9/tr:w-480,/b95c8ffMEERA-MER885_1.jpg?rnd=20200526195200",
  },

  {
    id: 20,
    productName: "Self Design High Neck Sweater For Men",
    description:
      "High Neck Sweaters can be included in your outfit for every occasion — whether it's work, a casual outing or a party.",
    productId: 7934368,
    quantity: 1,
    price: 750,
    mrp: 2499,
    discountDisplayLabel: "70% off",
    rating: 2.0,
    section: "Men",
    inStock: true,
    fastDelivery: false,
    trending: false,
    deliveryTime: "5 days",
    brand: "Kvetoo",
    category: "T-shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/3/d/tr:w-480,/3dfea3aHZ-S-BGREEN_1.jpg?rnd=20200526195200",
   
  },
  {
    id: 21,
    productName: "Red Hailey Dress",
    description:
      "Made of mess Tube dress Side high slit Ruffled tiered hemline Side conceal zip fully lyre lined",
    productId: 7741719,
    quantity: 1,
    price: 3943,
    mrp: 4150,
    discountDisplayLabel: "5% off",
    rating: 4.5,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "2 hour",
    brand: "Zara",
    category: "Dresses",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/0/a/tr:w-480,/0a9efb8IGHAIL_1.jpg?rnd=20200526195200",
    
  },

  {
    id: 22,
    productName: "Women Rani Cotton Tissue Saree with Unstitched Blouse",
    description:
      "Look and feel stylish in this rani pink saree from House of Begum. This saree is made of tissue silk fabric, which lets you look stylish and comfortable. Wear this saree with jhumkis and kolhapuris for an aced look.",
    productId: 6452608,
    quantity: 1,
    price: 1530,
    mrp: 4500,
    discountDisplayLabel: "66% off",
    rating: 4.0,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "6 hour",
    brand: "Saree mall ",
    category: "Saree",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/b/b/tr:w-480,/bb9c4f0TC1_1.jpg?rnd=20200526195200",
   
  },

  {
    id: 23,
    productName: "So High - Beige Dress-Beige",
    description:
      "Ruched bodycon mini dress with mesh fitted sleeves. Fully lycra lined with four way stetch factor",
    productId: 6481844,
    quantity: 1,
    price: 3373,
    mrp: 3550,
    discountDisplayLabel: "5% off",
    rating: 4.0,
    section: "Women",
    inStock: true,
    fastDelivery: false,
    trending: true,
    deliveryTime: "2 days",
    brand: "Zara",
    category: "Dresses",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/0/a/tr:w-480,/0a0355aIGSO002_1.jpg?rnd=20200526195200",
    
  },

  {
    id: 24,
    productName: "Wild Berry Linen Casual Regular Shirt",
    description:
      "Crafted from one of Earthâ€™s friendliest fibres, Belgian linen, that comes from Belgian flax, a supercrop that requires zero manual irrigation, is 100% bio-degradable & enriches the soil as it grows.",
    productId: 5903990,
    quantity: 1,
    price: 4990,
    mrp: 4990,
    discountDisplayLabel: "0% off",
    rating: 4.8,
    section: "Men",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "30 minutes",
    brand: "Zara",
    category: "Shirt",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/e/d/tr:w-480,/eda07f9NYKAF_ANDAM00002251_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 25,
    productName: "Power Play Top-Red",
    description:
    "Boned corset with Fusing. Chiffon draped corset with back zipper",
    productId: 6481840,
    quantity: 1,
    price: 2328,
    mrp: 2450,
    discountDisplayLabel: "5% off",
    rating: 3.9,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "45 minutes",
    brand: "Zara",
    category: "Dresses",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/3/9/tr:w-480,/39ec53bNF_ITGIR00000331_1.jpg?rnd=20200526195200",
    
  },
  {
    id: 25,
    productName: "Hoodie",
    description:"Bright and vibrant hoodie for kids with a kangaroo pocket and a hood.Perfect for adding a cool and edgy touch to your kids outfits.",
    productId: 6481840,
    quantity: 1,
    price: 1500,
    mrp: 2450,
    discountDisplayLabel: "5% off",
    rating: 3.9,
    section: "Women",
    inStock: true,
    fastDelivery: true,
    trending: true,
    deliveryTime: "45 minutes",
    brand: "Zara",
    category: "Dresses",
    image:
      "https://adn-static1.nykaa.com/nykdesignstudio-images/pub/media/catalog/product/3/9/tr:w-480,/39ec53bNF_ITGIR00000331_1.jpg?rnd=20200526195200",

    id: 26,
    productName: "Hoodie",
    description:
      "Bright and vibrant hoodie for kids with a kangaroo pocket and a hood.Perfect for adding a cool and edgy touch to your kids outfits.",
    category: "Kids' fashion",
    section: "Kids",
    rating: 4.5,
    section: "kids",
    price: 1500,
    originalPrice: 1800,
    image:
      "https://res.cloudinary.com/dptfwcnro/image/upload/v1684621708/E-comm%20ATTIREX/Kids-Fashion/hmgoepprod_zvhtpt.webp",
    isOutOfStock: false,
    isTrending: false,
  },
  {
    id: 27,
    productName: "Printed T-Shirt",
    category: "Kids' fashion",
    section: "Kids",
    rating: 4.8,
    section: "kids",
    price: 800,
    originalPrice: 1000,
    image:
      "https://res.cloudinary.com/dptfwcnro/image/upload/v1684160795/E-comm%20ATTIREX/Kids-Fashion/istockphoto-1356594834-612x612_p1ywwg.jpg",
    isOutOfStock: false,
    description:
      "Fun and playful printed t-shirt for kids with short sleeves and a crew neck.Perfect for adding a cool and edgy touch to your kids outfits.",
    isTrending: true,
  },
  {
    id: 28,
    productName: "Denim Overalls",
    category: "Kids' fashion",
    section: "Kids",
    rating: 4.6,
    section: "kids",
    price: 2000,
    originalPrice: 2500,
    image:
      "https://res.cloudinary.com/dptfwcnro/image/upload/v1684160742/E-comm%20ATTIREX/Kids-Fashion/istockphoto-1296361252-612x612_rkq0ow.jpg",
    isOutOfStock: true,
    description:
      "Stylish denim overalls for kids with adjustable straps and multiple pockets.Perfect for adding a cool and edgy touch to your kids outfits.",
    isTrending: false,
  },
  {
    id: 29,
    productName: "Striped Dress",
    category: "Kids' fashion",
    section: "Kids",
    rating: 4.2,
    section: "kids",
    price: 1800,
    originalPrice: 2000,
    image:
      "https://res.cloudinary.com/dptfwcnro/image/upload/v1684160787/E-comm%20ATTIREX/Kids-Fashion/istockphoto-1361311726-612x612_cnjg6c.jpg",
    isOutOfStock: false,
    description:
      "Adorable striped dress for kids with a flared skirt and a bow accent.Perfect for adding a cool and edgy touch to your kids outfits.",
    isTrending: false,
  },
];